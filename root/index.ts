...
do {
  ...
  if (answer.operator === "x raise to the power y") {
    console.log(
      chalk.blueBright(
        `${answer.firstNumber} raise to the power ${answer.secondNumber} is: ${generatedVal}`
      )
    );
  } else {
...
function calculate(opt: string, num1: number, num2: number): number {
  switch (opt) {
    case "Addition":
      return num1 + num2;
    case "Subtraction":
